...
class Game(PygameGame):
    ...
    def changeBGMusic(self):
        self.bgMusic = pygame.mixer.Sound('music/videoMusic.wav')
        self.playBGMusic()
        ...
        while (serverMsg.qsize() > 0):
            ...
            try:
                ...
                if (command == "myIDis"):
                    ...
                    # self.win = 1
                    # self.gameState = "endState"
                elif (command == "GenerateEnemyType1"):
                    # print("Process GenerateEnemyType1")
                    (x, y, self.round) = (int(msg[2]), int(msg[3]), int(msg[4]))
                    self.enemiesType1.add(EnemyType1(x, y, self.round))
                elif (command == "GenerateEnemyType2"):
                    (x, y, self.round) = (int(msg[2]), int(msg[3]), int(msg[4]))
                    self.enemiesType2.add(EnemyType2(x, y, self.round))
                elif (command == "GenerateEnemyType3"):
                    print("Processing enemy 3")
                    (x, y, self.round) = (int(msg[2]), int(msg[3]), int(msg[4]))
                    self.enemiesType3.add(EnemyType3(x, y, self.round))
                elif (command == "GenerateFirstAidStation"):
                    (x, y) = (int(msg[2]), int(msg[3]))
                    self.firstAidStations.add(FirstAidStation(x, y))
                elif (command == "GenerateWeaponSupplyStation"):
                    (x, y) = (int(msg[2]), int(msg[3]))
                    self.weaponSupplyStations.add(WeaponSupplyStation(x, y))
                elif (command == "GenerateBombSupplyStation"):
                    (x, y) = (int(msg[2]), int(msg[3]))
                    self.bombSupplyStations.add(BombSupplyStation(x, y))
                elif (command == "GenerateEnemyStation"):
                    (x, y) = (int(msg[2]), int(msg[3]))
                    self.enemyStations.add(EnemyStation(x, y))
                elif (command == "GenerateBoss"):
                    (x, y) = (int(msg[2]), int(msg[3]))
                ...
                elif (command == "EneterBossState1"):
                    boss = self.bossType1Group.sprites()[0]
...
####################################        
    ...
    def endStateRedrawAll(self, screen):
        if self.win == None:
            ...
            screen.blit(textSurf, textRect)
            if self.round == 4:
                (msg, color) = ("You won! You reached the 112 planet!", (224, 255, 255))
                textSurf, textRect = self.text_objects(msg, color)
                textRect.center = (self.width//2, self.height*5//8)
                screen.blit(textSurf, textRect)
...
#################################### 
    ...
    def gameStateRedrawAll(self, screen):      
        if self.gameState == "gameState" or self.gameState == "in boss fight":
            ...
            # draw type 1, type 2, type 3 enemies
